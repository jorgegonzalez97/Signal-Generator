% codigoT2.m
%
% Genera las fases del código T2 de longitud T.
%
...
function [phase, phaseUnWrapped, t, error] = codigoT2(fs,T,muestras,n,k)
    ...
    for i = 1:length(t)
        ...
        phase(i) = mod(((2*pi/n)*floor((k*t(i)-jj*T)*((2*jj-k+1)/T)*(n/2))),2*pi);
        % fase desenrollada
        if i==1
